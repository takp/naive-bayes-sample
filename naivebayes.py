...
# Use yahoo Japanese Morphologial Anylysis API
...
class NaiveBayes:
	def __init__(self):
		self.vocabularies = set() # a set of words (単語の集合)
		self.wordcount = {} 			# dictionary { category : { words : n, ... } }
		self.catcount = {}				# dictionary { category : n }

	# Count up the words in category
	...
	# Return the counts that word shows up in one category
	...
	# Add-one smoothing (for zero frequency problem)
	# 1を加えて、加算スムージングを行う。（出現回数０の単語をなくす）
	...
	# Use logarithm because the probability is too small number (underflow)
	# 確率値の対数を算出する（確率値が小さくなりすぎるため）
	# 対数を取って、積ではなく、和の計算に変換する
...
# Test Data (expect return Python)
words = u'オランダ人のヴァンロッサム氏によって開発されました.'
print u'%s => 推定カテゴリ: %s' % (words ,nb.classifier(words))

words = u'豊富なドキュメントや豊富なライブラリがあります.'
print u'%s => 推定カテゴリ: %s' % (words ,nb.classifier(words))

# Test Data (expect return Ruby)
